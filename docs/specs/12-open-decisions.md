...
- [x] **Vulnérabilité Next.js 14.2.0**
      → **Décision (post-sprint-8, 2026-04-29) :** bump `next@14.2.35`
      + `eslint-config-next@14.2.35`. Résout les CVEs critiques (auth
      bypass, request smuggling, DoS Server Components). `npm audit`
      passé de 13 vulns (1 critique, 6 high) à 10 vulns (0 critique,
      4 high — toutes en deps dev/transitives). Tests 130/130 verts,
      typecheck strict OK, lint clean. Rester sur la branche 14.2.x
      (bump majeur 16.x = breaking).
